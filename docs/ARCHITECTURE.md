...
| `src/sender/display.ts`            | `SenderDisplay`: rAF loop, canvas sizing (device pixels), tick → esi selection → QR encode → compose → `putImageData`; META tile every 32 ticks; stats every ~500 ms; wake lock held automatically; `dispose()` frees the encoder                                                                                                                                                                                                                                                                                          | The broadcast core; encodes **complete wire frames** (header+payload+CRC), not payloads                                          |

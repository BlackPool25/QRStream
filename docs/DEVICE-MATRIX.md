...
- Sender, ready: "Ready to broadcast", fields `File` / `Size` / `Profile` / `Symbols`, where Profile is `GRID 2×2` (the UI always uses the 2×2 grid; the `SINGLE V40` profile exists in the protocol layer as the low-resolution fallback but is not selectable from the app), and Symbols is `k = N`.
- Sender, broadcasting: chips for filename, size, profile (`GRID 2×2`), `k N`, `N.N fps`, `N dropped`, and elapsed time. Buttons: `Fullscreen`, `Boost`, `Stop`.
...
4. **Send**: `SEND` → `Choose a file` → pick a fixture. Wait for "Ready to broadcast" (shows `GRID 2×2`, `k = N`). `Begin broadcast`. The QR grid is visible; `Fullscreen` works (iOS supports it since 12); `Boost` is a no-op if wake lock is unavailable (it silently stays off, which is expected).
...
`docs/PERF.md` does not exist yet (the performance task runs in parallel). Until
it lands, treat these as the expected band and cite this doc; once `docs/PERF.md`
is generated, update this table with its measured numbers.

| Payload              | Expected time                                                                       | Notes                                                                                       |
| -------------------- | ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| 1 MB                 | ~20-60 s                                                                            | At grid throughput; smaller text/PNG files are much faster (single-digit seconds).          |
| 10 MB                | ~3-10 min                                                                           | Longest practical transfer; battery/brightness matter, keep the sender plugged or boost on. |
| Effective throughput | single V40 ~15-35 KB/s; 2×2 grid ~35-70 KB/s best case; phone-to-phone tends higher | The UI sends the grid profile, so expect the high end.                                      |

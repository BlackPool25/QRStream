...
3. **ML Kit R8/AGP 9 keep rules (this session)**: the release build crashed at scan time with "getClass() on a null object reference" (iQOO Z9 5G / OriginOS) — AGP 9's R8 full mode strips ML Kit's reflectively loaded bundled-model classes (`google_mlkit_barcode_scanning` ships no consumer rules). Fixed with keep rules in `android/app/proguard-rules.pro` (`com.google.mlkit.vision.barcode.**`, `mlkit_vision_barcode_bundled.**`, `mlkit_vision_barcode.**`, `mlkit_vision_common.**`). The Flutter Gradle plugin auto-discovers this file and adds it to the release R8 rules — **do NOT add a `proguardFiles(...)` line to the release build type**: it replaces the Flutter plugin's injected rules, R8 strips every plugin class, and the app crashes at launch in the reflected `GeneratedPluginRegistrant` (`NoClassDefFoundError`) — debug worked, release crashed. No dependency exclusions are needed — the plugin's `com.google.mlkit:barcode-scanning:17.3.0` is the bundled-model variant and scanning runs in-process, fully offline. Both are templated (`android_templates/build.gradle.kts.template`, `android_templates/proguard-rules.pro`); a fresh clone must re-apply both templates after `flutter create`. Also: plugin upgraded to `google_mlkit_barcode_scanning 0.15.0`, camera bumped to 1080p (`ResolutionPreset.veryHigh`), and `MlKitFrameDecoder` gained a dead-I420→YV12 fix plus a zxing2 `DecodePool` last-resort fallback.
4. Keep PWA green (`display.ts` has an uncommitted parallel-task change — do not touch).

...
void main() {
  ...
  group('BroadcastController', () {
    ...
    testWidgets('setLayout re-flows the grid mid-broadcast', (tester) async {
      final controller = await spawnController(tester); // default grid4/15
      controller.start();
      expect(controller.layout, LayoutId.grid4);
      expect(controller.tilesPerFrame, 4);

      await tester.pump(const Duration(milliseconds: 67)); // sets the start
      await tester.pump(const Duration(milliseconds: 67)); // frame 0 (meta)
      expect(controller.currentFrame, hasLength(4));

      // Re-flow to a dual-lane layout mid-broadcast: grid geometry, schedule
      // branch and fps ceiling all follow the LIVE layout.
      controller.setLayout(LayoutId.row2);
      expect(controller.layout, LayoutId.row2);
      expect(controller.tilesPerFrame, 2);
      expect(
        controller.settings.layout,
        LayoutId.grid4,
        reason: 'settings are immutable transfer config',
      );
      ...
      // The next rendered frame carries the new tile count + dual-lane schedule.
      controller.renderFrame(controller.tickCount);
      expect(controller.currentFrame, hasLength(2));
      expect(controller.lastEsis, hasLength(2));
      final k = sharedPrepared.info.k;
      final repair = (k * repairExtraFactor).ceil() + repairExtraMin;
      expect(
        controller.lastEsis,
        nextEsiDualLane(k, repair, controller.tickCount - 1),
      );

      // Re-flow back to a grid layout: round-robin resumes at the new tile
      // count (3 tiles, data-only frame).
      controller.setLayout(LayoutId.column3);
      expect(controller.layout, LayoutId.column3);
      expect(controller.tilesPerFrame, 3);
      controller.renderFrame(controller.tickCount);
      expect(controller.currentFrame, hasLength(3));
      expect(controller.lastFrameMeta, isFalse);
      expect(controller.lastEsis, hasLength(3));

      controller.stop();
      controller.dispose();
    });

    testWidgets('setLayout clamps fps to the new layout ceiling', (
      tester,
    ) async {
      const settings = TransferSettings(
        bytesPerTile: BytesPerTileId.oneK,
      ...
      expect(
        ...
        30,
        reason: 'grid4 caps at 30 on the 90Hz+ path',
      );

      // grid9 caps at 24 regardless of the display.
      controller.setLayout(LayoutId.grid9);
      expect(controller.layout, LayoutId.grid9);
      expect(controller.tilesPerFrame, 9);
      expect(controller.currentFps, 24, reason: 'grid9 ceiling clamps fps');

      // Back on a 30-cap layout the cadence recomputes up to the target.
      controller.setLayout(LayoutId.row3);
      expect(controller.currentFps, 30, reason: 'row3 ceiling restores 30');
